...
void main() {
  ...
  const resumeId = 'sharpline-template-preview';

  ResumeModel buildResume() {
    final now = DateTime(2026, 3, 12);
    return ResumeModel(
      id: resumeId,
      title: 'SharpLine Preview Resume',
  ...
  Future<void> openSharpLineDialog(WidgetTester tester) async {
    final carousel = find.byKey(const ValueKey('template-carousel'));
    final card = find.byKey(const ValueKey('template-card-modern_aesthetic'));
    final previewButton = find.byKey(
      const ValueKey('template-preview-modern_aesthetic'),
    );
  ...
  setUpAll(() async {
    ...
    hiveDir =
        await Directory.systemTemp.createTemp('resume-app-sharpline-preview');
  ...
  testWidgets('sharp line preview dialog uses the stored custom sections', (
    tester,
  ) async {
    await pumpTemplateScreen(tester);
    await openSharpLineDialog(tester);
