...
class ForestEdgeContactItem {
  const ForestEdgeContactItem({
    required this.kind,
    required this.label,
  });

  final ForestEdgeContactKind kind;
  final String label;
}

class ForestEdgeEducationEntry {
  const ForestEdgeEducationEntry({
...
class ForestEdgeExperienceEntry {
  const ForestEdgeExperienceEntry({
...
class ForestEdgeProjectEntry {
  const ForestEdgeProjectEntry({
    required this.title,
    this.detailLines = const <String>[],
...
class ForestEdgeCertificationEntry {
  const ForestEdgeCertificationEntry({
    required this.name,
    this.detailLines = const <String>[],
    this.links = const <String>[],
  });
...
class ForestEdgeTemplateSupport {
  static const int pageBgHex = 0xFFF1F2F3;
  static const int paperHex = 0xFFF8F8F8;
  static const int cardHex = 0xFFF5F6F7;
  static const int headerTintHex = 0xFFDCE2E8;
  static const int headerAccentHex = 0xFFC9D3DE;
  static const int lineHex = 0xFFD5D9DE;
  static const int inkHex = 0xFF39424C;
  static const int mutedHex = 0xFF6E7680;
  static const int softHex = 0xFF9AA2AC;
  static const int badgeHex = 0xFFE5E7EA;
  ...
  static List<ForestEdgeContactItem> contactItems(
    PersonalInfo? info, {
    bool compactLinks = true,
    bool includeAddress = true,
  }) {
    final items = <ForestEdgeContactItem>[];
    final seen = <String>{};

    void add(ForestEdgeContactKind kind, String? value) {
      ...
      items.add(ForestEdgeContactItem(kind: kind, label: label));
    }

    add(ForestEdgeContactKind.phone, info?.phone);
    if (includeAddress) {
      add(ForestEdgeContactKind.address, info?.address);
    }
    add(ForestEdgeContactKind.email, info?.email);
    add(ForestEdgeContactKind.linkedin, info?.linkedIn);
    add(ForestEdgeContactKind.github, info?.github);
    add(ForestEdgeContactKind.website, info?.website);
  ...
  static List<ForestEdgeEducationEntry> educationEntries(
    ...
    final entries = educations.map((education) {
      ...
      return ForestEdgeEducationEntry(
  ...
  static List<ForestEdgeExperienceEntry> experienceEntries(
    ...
    final entries = experiences.map((experience) {
      ...
      return ForestEdgeExperienceEntry(
  ...
  static List<ForestEdgeProjectEntry> projectEntries(
  ...
  static List<ForestEdgeCertificationEntry> certificationEntries(
    ...
    final entries = certifications
        .map((certification) {
          ...
          return ForestEdgeCertificationEntry(
            name: name,
            detailLines: List.unmodifiable(detailLines),
            links: List.unmodifiable(links),
          );
        })
        .whereType<ForestEdgeCertificationEntry>()
        .toList(growable: false);
