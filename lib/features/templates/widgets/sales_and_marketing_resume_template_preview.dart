...
class SalesAndMarketingResumeTemplatePreview extends StatelessWidget {
  const SalesAndMarketingResumeTemplatePreview({
    super.key,
  ...
  List<_SalesPreviewContactItem> get _contactItems {
    final info = resume?.personalInfo;
    final items = <_SalesPreviewContactItem>[];

    void add(IconData icon, String? value) {
      final trimmed = value?.trim() ?? '';
      if (trimmed.isNotEmpty) {
        items.add(_SalesPreviewContactItem(icon: icon, label: trimmed));
      }
    }
  ...
  List<_SalesPreviewExperience> get _experiences {
    final values = resume?.experience ?? const <Experience>[];
    if (values.isEmpty) {
      return const [
        _SalesPreviewExperience(
    ...
    return values.take(2).map((experience) {
      ...
      return _SalesPreviewExperience(
  ...
  _SalesPreviewEducation get _education {
    final values = resume?.education ?? const <Education>[];
    if (values.isEmpty) {
      return const _SalesPreviewEducation(
  ...
  List<_SalesPreviewProject> get _projects {
    final values = resume?.projects ?? const <Project>[];
    if (values.isEmpty) {
      return const [
        _SalesPreviewProject(
  ...
  Widget _sectionHeader(String title) {
    return Padding(
      ...
      child: Row(
        ...
        children: [
          _text(title, size: 4.35, color: _ink, weight: FontWeight.w700),
  ...
  Widget _headerContactItem(_SalesPreviewContactItem item) {
    return SizedBox(
      height: 10,
      child: Row(
        children: [
          ...
          Expanded(
            child: Text(
              ...
              style: TextStyle(fontSize: 2.55, color: _muted, height: 1.05),
            ),
          ),
        ],
      ),
  ...
  List<List<_SalesPreviewContactItem>> get _headerContactRows {
    if (_contactItems.isEmpty) {
      return const [];
    }

    final rows = <List<_SalesPreviewContactItem>>[];
  ...
  Widget _experienceBlock(_SalesPreviewExperience experience) {
    ...
    return Padding(
      ...
      child: Column(
        ...
        children: [
          ...
          _text(experience.companyLine, size: 3.05, color: _muted, maxLines: 2),
          if (highlights.isNotEmpty) ...[
            const SizedBox(height: 1.1),
            _bulletLines(highlights, size: 2.95, color: _muted, maxLines: 2),
          ],
        ],
      ),
    );
  ...
  Widget _projectBlock(_SalesPreviewProject project) {
    return Padding(
      ...
      child: Column(
        children: [
          ...
          Padding(
            ...
            child:
                Container(height: 0.9, color: _divider.withValues(alpha: 0.9)),
          ),
          Expanded(
            child: Padding(
                ...
                child: Column(
                  ...
                  children: [
                    if (_summaryLines.isNotEmpty) ...[
                      ...
                      _bulletLines(_summaryLines, size: 3.05, color: _muted),
                    ],
...
class _SalesPreviewContactItem {
  const _SalesPreviewContactItem({required this.icon, required this.label});

  final IconData icon;
  final String label;
}

class _SalesPreviewExperience {
  const _SalesPreviewExperience({
    required this.title,
...
class _SalesPreviewEducation {
  const _SalesPreviewEducation({
...
class _SalesPreviewProject {
  const _SalesPreviewProject({required this.title, required this.description});
